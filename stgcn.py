...
class SpatialBlock(nn.Module):
    ...
    def forward(self, X, A_hat):
        ...
        lfs_cpu = lfs.detach().cpu().numpy()
        t2 = F.relu(torch.matmul(lfs, self.Theta1))
        # rfs=torch.einsum("jklm,ij->kilm", [X.permute(1, 0, 2, 3),A_hat])
        #
...
class STGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.T_block1 = TimeBlock(in_channels=7, out_channels=14)
        self.batch_norm1 = BatchNormBlock(num_features=int(2))
        self.T_block2 = TimeBlock(in_channels=14, out_channels=28)
        self.batch_norm2 = BatchNormBlock(num_features=int(2))
        # self.T_block3 = TimeBlock(in_channels=7, out_channels=1)
        # self.batch_norm3 = BatchNormBlock(num_features=int(2))
        # self.last_temporal = TimeBlock(in_channels=12, out_channels=1)
        self.S_block1=SpatialBlock(spatial_channels=14, out_channels=28,
                 num_nodes=num_nodes)
        self.S_block2=SpatialBlock(spatial_channels=7, out_channels=14,
                 num_nodes=num_nodes)
        self.fully = nn.Linear(2*7*num_nodes,
                               num_class)
